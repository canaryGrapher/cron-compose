...
configure_database() {
  ...
  local default_method="${CC_DB_METHOD:-existing}"
  if [ "${NONINTERACTIVE:-0}" != "1" ]; then
    info "1) Use an existing Postgres (enter a connection string)"
    [ "$HAVE_PSQL" = "1" ]   && info "2) Auto-create a local database with psql"
    [ "$HAVE_DOCKER" = "1" ] && info "3) Run Postgres in Docker (docker-compose.yml)"
    local choice
    choice="$(prompt "Select" "1")"
    case "$choice" in
      2) [ "$HAVE_PSQL" = "1" ] && default_method="psql" || default_method="existing" ;;
      3) [ "$HAVE_DOCKER" = "1" ] && default_method="docker" || default_method="existing" ;;
